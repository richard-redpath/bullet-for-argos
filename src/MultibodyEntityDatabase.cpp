...
/*
 * This was intended to calculate initial global coordinate locations before switching to
 * another method
 *
 *
	//
	// Now we need to calculate the position of each element
	//

	//
	// We start with dependency resolution
	//

	// Create an ordered list of all links
	Link* allLinks[links.size()];

	// Add all links to this list
	Link** linkIter = allLinks;
	for(auto& p : links)
		(*linkIter++) = &p.second;

	// Track link parents (dependencies) with associated joints
	std::map<Link*, std::pair<JointDefinition*, Link*>> linkParents;

	// Setup dependencies
	for(auto& p : joints)
	{
		Link* parent = &links[p.second.parent];
		Link* child = &links[p.second.child];
		linkParents[child] = std::pair<JointDefinition*, Link*>(&p.second, parent);
	}

	// Now we can resolve them
	Link** checking = allLinks;				// This is in the unresolved section
	Link** firstUnresolved = allLinks;		// This is immediately after the resolved section
	std::set<Link*> resolved;

	// Keep going until we try to check something out of bounds
	while(checking < allLinks + links.size())
	{
		// Get the dependencies for this link
		auto currentDeps = &linkParents[*checking];

		//  Does this link have a parent?       Has it been resolved?
		if((currentDeps->second == nullptr) || (resolved.find(currentDeps->second) != resolved.end()))//std::includes(resolved.begin(), resolved.end(), currentDeps->begin(), currentDeps->end()))
		{
			// If so then add it to the resolved items
			resolved.insert(*checking);

			// Move the link we are checking to the end of the resolved section
			Link* tmp = *checking;
			*checking = *firstUnresolved;
			*firstUnresolved = tmp;

			// Update where the next unresolved item is
			firstUnresolved++;

			// And scan again from there
			checking = firstUnresolved;
		}
		else
		{
			// This item has not yet been resolved so try the next one
			checking++;
		}
	}

	// Check that all links have had there dependencies met before we attempt to position links
	if(resolved.size() != links.size())
	{
		std::cerr<<"Unable to find valid ordering for link position resolution. Ensure that there are no cycles in entity model \""<<name<<"\""<<std::endl;
		exit(1);
	}

	// The first element must be the root as a result of the dependency resolution so we do not need to position it
	for(int positionIdx = 1; positionIdx < links.size(); ++positionIdx)
	{
		// Get the current link
		Link* positioning = allLinks[positionIdx];

		auto locationInfo = linkParents[positioning];
		positioning->originX = locationInfo.second->originX + locationInfo.first->originX;
		positioning->originY = locationInfo.second->originY + locationInfo.first->originY;
		positioning->originZ = locationInfo.second->originZ + locationInfo.first->originZ;

		CQuaternion q1 = CQuaternion();
		q1.FromEulerAngles(CRadians{locationInfo.second->yaw},
						   CRadians{locationInfo.second->pitch},
						   CRadians{locationInfo.second->roll});


		CQuaternion q2 = CQuaternion();
		q2.FromEulerAngles(CRadians{locationInfo.first->originYaw},
						   CRadians{locationInfo.first->originPitch},
						   CRadians{locationInfo.first->originRoll});

		q2 *= q1;

		CRadians roll, pitch, yaw;
		q2.ToEulerAngles(yaw, pitch, roll);

		positioning->roll = roll.GetValue();//locationInfo.second->roll + locationInfo.first->originRoll;
		positioning->pitch = pitch.GetValue();//locationInfo.second->pitch + locationInfo.first->originPitch;
		positioning->yaw = yaw.GetValue();//locationInfo.second->yaw + locationInfo.first->originYaw;
	}
// */
}

...
//		std::vector<char> buf(maxchars); // Alloc enough size.
	...
	void write_min(std::ostream &stream,
				   const std::vector<tinyobj::shape_t> &shapes)
	{
		...
		for(size_t i = 0; i < nMeshes; ++i)
		{
			...
			const uint32_t nameLen = name.length();
			...
			stream.write((const char *) &nameLen, sz);
...
//			stream.write((const char*)&shapes[i].material.ambient[0], 3 * sz);
	...
	std::vector<tinyobj::shape_t> read_min(std::istream &stream)
	{
		assert(sizeof(float) == sizeof(uint32_t));
		const auto sz = sizeof(uint32_t);
...
//		std::string minFileName {filename};
//		minFileName += ".min";
//
//		struct stat mainStat, minStat;
//		stat(filename, &mainStat);
//		stat(filename, &minStat);
//
//		std::ifstream minInFile(minFileName);
//		// If the min file already exists and is newer than the main file
//		if(minInFile && (minStat.st_mtim.tv_nsec > mainStat.st_mtim.tv_nsec))
//		{
//			shapes = read_min(minInFile);
//			return "";
//		}
		...
		std::string ret = LoadObj(shapes, materials, ifs, matFileReader);

		if(ret.empty())
		{
//			std::ofstream outFile(minFileName);
//			std::cout<<(minInFile ? "Over-" : "")<<"Writing min file"<<std::endl;
//			write_min(outFile, shapes);
//			outFile.flush();
		}
